...
class Net(torch.nn.Module):
    ...
    def get_loss(self, predict, labels, labels_length):
        ...
        if self.word:
            ...
            seq_len = torch.tensor([log_predict.shape[0]] * log_predict.shape[1], dtype=torch.long)
            loss = self.loss(log_predict.cpu(), labels.long(), seq_len, labels_length.long())
    ...
    def export_onnx(self, net, dummy_input, graph_path, input_names, output_names, dynamic_ax):
        # torch 2.x dynamo 导出器会把权重写成外部 .onnx.data 文件, 分发时容易遗漏;
        # 用 dynamo=False(旧版导出器) 产出单文件 onnx, 便于随 charsets.json 一起分发
    ...
    def load_checkpoint(path, device):
        # torch 2.6+ 默认 weights_only=True, 旧版 checkpoint 含非张量对象需关闭
        param = torch.load(path, map_location=device, weights_only=False)
