...
**`--model` 模式的行为**：`src/main.py` 会同时展示**通用模型**与**自定义模型**的全部候选。候选标签中带 `(自定义)` 的即为专用模型输出。**有专用模型结果时直接以它为首选**（专用模型按该类验证码专门训练，实测远强于内置多变体投票）；无自定义结果时才退回内置 ddddocr 的多变体择优。不传 `--model` 时行为不变，仍只用内置 ddddocr。
